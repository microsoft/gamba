...
class CaduceusConservationForMaskedLM(CaduceusPreTrainedModel):
    """Caduceus model with added conservation head for masked LM + continuous value prediction."""
    ...
    def __init__(self, config: CaduceusConfig, device=None, dtype=None, **kwargs):
        ...
        self.lm_head = RCPSLMHead(
            ...
            true_dim=config.d_model,
            **factory_kwargs
        ) if config.rcps else nn.Linear(config.d_model, config.vocab_size, bias=False, **factory_kwargs)
    ...
    def forward(
        ...
        if labels is not None:
            ce_loss = weighted_cross_entropy(logits, labels, loss_weights, ignore_index=-100) \
                if loss_weights is not None else cross_entropy(logits, labels, ignore_index=-100)
