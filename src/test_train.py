...
ckpt_dir = os.getenv("AMLT_OUTPUT_DIR", "/home/mica/scratch/dcps/normalized/") + "/"
...
def get_dataloader(
    ...
    if is_amlt():
        ...
        data_top_dir = args.data_root or "/home/mica/scratch/gamba/data_processing/data/"
    ...
    if RANK == 0 and not args.no_wandb:
        ...
        if os.path.exists(run_id_file):
            with open(run_id_file, "r") as f:
                run_id = f.read().strip()
            resume_mode = "must"  # fail fast if the ID doesn't exist on W&B
        else:
            run_id = None
            resume_mode = "allow"  # create on first run
        ...
        run = wandb.init(
            ...
            # settings=wandb.Settings(start_method="thread"),  # optional on some clusters
        )
        ...
        # Optional: make W&B’s x-axis follow your step key without adding step=...
        wandb.define_metric("nsteps")
