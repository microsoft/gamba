...
def calculate_log_likelihood(sequence_tokens, scores, model, collator, device, mutation_pos):
    ...
    print(f"type of collated: {type(collated)}")
    with torch.no_grad():
        output = model(collated[0].to(device), collated[1].to(device))
    logits = output["seq_logits"]
    if mutation_pos + 1 >= logits.size(1):
        raise IndexError(f"mutation_pos + 1 ({mutation_pos + 1}) is out of bounds for logits with size {logits.size(1)}")
    log_probs = torch.nn.functional.log_softmax(logits[:, mutation_pos + 1, :], dim=-1)
    print(f"shapes of sequence and logits: {seq_lbls.shape}, {log_probs.shape}")
    
    if mutation_pos + 1 >= seq_lbls.shape[1]:
        raise IndexError(f"mutation_pos + 1 ({mutation_pos + 1}) is out of bounds for seq_lbls with size {seq_lbls.shape[1]}")
    
    # Extract the labels at the mutation position
    indices = seq_lbls[:, mutation_pos + 1].long()  # Ensure indices are int64
    print("indices:", indices)

    print("mutation position:", mutation_pos)
    print(f"sequence length: {(seq_lbls.size(1))}")
    
    # Check for invalid indices
    if (indices < 0).any() or (indices >= log_probs.size(1)).any():
        print(f"Invalid indices found: {indices}")
        print(f"sequence_tokens: {sequence_tokens}")
        print(f"scores: {scores}")
        print(f"mutation_pos: {mutation_pos}")
...
def calculate_average_log_likelihood(sequence_tokens, scores, model, collator, device, mutation_pos):
    # Calculate log-likelihood for the original sequence
    log_likelihood = calculate_log_likelihood(sequence_tokens, scores, model, collator, device, mutation_pos)
    
    # if sequence contains "4" which is "N"
    if 4 in sequence_tokens:
        print("Sequence contains 'N' characters, skipping reverse complement calculation")
        return log_likelihood

    # Reverse complement the sequence
    sequence_token_tensor = torch.tensor(sequence_tokens, dtype=torch.long)
    scores_tensor= torch.tensor(scores, dtype=torch.float32)
    reverse_complement_sequence = reverse_complement(sequence_token_tensor)
    reverse_complement_scores = scores_tensor.flip(dims=[0])

    print(f"length of reverse_complement_sequence: {len(reverse_complement_sequence)}")
    print(f"length of reverse_complement_scores: {len(reverse_complement_scores)}")
    

    reverse_complement_sequence = reverse_complement_sequence.numpy()
    reverse_complement_scores = reverse_complement_scores.numpy()

    print(f"length of reverse_complement_sequence: {len(reverse_complement_sequence)}")
    print(f"length of reverse_complement_scores: {len(reverse_complement_scores)}")
    
    # Calculate log-likelihood for the reverse complement sequence
    reverse_log_likelihood = calculate_log_likelihood(reverse_complement_sequence, reverse_complement_scores, model, collator, device, mutation_pos)
    
    # Average the log-likelihoods
    average_log_likelihood = (log_likelihood + reverse_log_likelihood) / 2
    return average_log_likelihood

def process_variants(csv_file, bigwig_file, genome_fasta, model, collator, tokenizer, device, batch_size=36):
    ...
    for index, row in df.iterrows():
        ...
        if chromosome not in valid_chromosomes:
            ...
            continue
        position = int(row['Lifted_Position'])
        if "clin_var_GPNMSA.csv" in csv_file:
            label = row['label'].split('_')[0]
            ref = row['reference']
            alt = row['alternate']
        else:
            label = row['label']
        ...
        if len(sequences) == batch_size:
            common_log_likelihood_ratios_batch, pathogenic_log_likelihood_ratios_batch = process_batch(sequences, scores_list, labels, model, collator, device, mutation_pos)
            common_log_likelihood_ratios.extend(common_log_likelihood_ratios_batch)
            pathogenic_log_likelihood_ratios.extend(pathogenic_log_likelihood_ratios_batch)
            sequences = []
            scores_list = []
            labels = []
...
def process_batch(sequences, scores_list, labels, model, collator, device, mutation_pos):
    dataset = SequenceDataset(sequences, scores_list)
    dataloader = DataLoader(dataset, batch_size=len(sequences), collate_fn=collator)

    common_log_likelihood_ratios = []
    pathogenic_log_likelihood_ratios = []

    for batch_sequences, batch_scores in dataloader:
        batch_sequences = batch_sequences.to(device)
        batch_scores = batch_scores.to(device)

        for i in range(len(batch_sequences)):
            sequence_tokens = batch_sequences[i].cpu().numpy()
            scores = batch_scores[i].cpu().numpy()
            label = labels[i]

            ref_log_likelihood = calculate_average_log_likelihood(sequence_tokens, scores, model, collator, device, mutation_pos)

            # Create the mutated sequence
            mutated_sequence = list(sequence_tokens)
            ...
            mutated_sequence = np.array(mutated_sequence)

            mutated_log_likelihood = calculate_average_log_likelihood(mutated_sequence, scores, model, collator, device, mutation_pos)
...
def plot_log_likelihood_ratios(common_log_likelihood_ratios, pathogenic_log_likelihood_ratios, output_file, mutation_file):
    ...
    # Plot histogram for common scores
    plt.hist(common_log_likelihood_ratios, bins=50, alpha=0.5, label='Common', color='blue', log=True)
    
    # Plot histogram for pathogenic scores
    plt.hist(pathogenic_log_likelihood_ratios, bins=50, alpha=0.5, label='Pathogenic', color='red', log=True)
...
def main():
    ...
    common_log_likelihood_ratios, pathogenic_log_likelihood_ratios = process_variants(args.csv_file, args.big_wig, args.genome_fasta, model, collator, tokenizer, device)
