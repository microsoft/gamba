...
def load_config_and_model(
    ...
    if task == TaskType.OADM:
        ...
        model = CaduceusConservationForMaskedLM(caduceus_config)
...
def get_latest_dcp_checkpoint_path(ckpt_dir: str, last_step: int = -1) -> Optional[str]:
    ...
    if last_step == -1:
        ...
        for dir_name in os.listdir(ckpt_dir):
            if "dcp_conscaduceus" in dir_name:
                step = int(dir_name.split("dcp_conscaduceus")[-1])
                if step > last_step:
                    ckpt_path = os.path.join(ckpt_dir, dir_name)
                    last_step = step
    else:
        ckpt_path = os.path.join(ckpt_dir, f"dcp_conscaduceus{last_step}")
    return ckpt_path
