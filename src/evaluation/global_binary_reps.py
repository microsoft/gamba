...
def make_upstream_region(region, genome, offset=2000):
    """
...
def analyze_upstream_pairs(
    ...
    for group_name, group_chroms in chromosome_groups.items():
        ...
        for category in categories:
            ...
            # collect base regions from BED files
            import glob
            ...
            for bed_file in bed_files:
                ...
                base_regions.extend([r for r in loaded if r["chrom"] in group_chroms])

            if not base_regions:
                logging.warning(f"    no regions for {category} in group {group_name}")
                continue

            base_regions = base_regions[:num_regions]
            logging.info(f"    using {len(base_regions)} base regions")

            # build paired contexts: roi + upstream
            valid_regions = []
            n_pairs = 0

            for r in base_regions:
                up = make_upstream_region(r, genome, offset=2000)
                if up is None:
                    continue
                ...
                pos_ctx = extract_context(bigwig_file, r, genome, ctx_model_type)
                neg_ctx = extract_context(bigwig_file, up, genome, ctx_model_type)
                ...
                for ctx, cls in ((pos_ctx, "roi"), (neg_ctx, "upstream")):
                    ctx["chrom"] = r["chrom"]
                    ctx["category"] = category
                    ctx["class_label"] = cls
                    # ensure start/end reflect the roi coordinates, not the window
                    if cls == "roi":
                        ctx["start"] = int(r["start"])
                        ctx["end"] = int(r["end"])
                    else:
                        ctx["start"] = int(up["start"])
                        ctx["end"] = int(up["end"])
