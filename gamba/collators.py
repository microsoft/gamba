...
from evodiff2.constants import FIM_MIDDLE, FIM_PREFIX, FIM_SUFFIX


def pad_to_mult(max_len, pad_to_mult):
    "helper function to pad to multiple of pad_to_mult"
    max_len = max_len if pad_to_mult is None else pad_to_mult * torch.ceil(max_len / pad_to_mult).to(dtype=torch.int)
    return max_len


class OAMaskCollator:
    def __init__(self, tokenizer: Callable, pad_to_multiple_of: Optional[int] = None) -> None:
        self.tokenizer = tokenizer
        self.pad_to_mult = pad_to_multiple_of

    def __call__(self, sequences: Sequence[tuple]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        input_tokens = torch.full((len(tokenized), max_len), self.tokenizer.pad_id, dtype=torch.long)
        ...
        timesteps = lens - torch.tensor(np.random.randint(1, [max(2, lt) for lt in lens])) + 1
...
class LMCollator:
    ...
    def make_splitter(min_prefix_len: int, min_suffix_len: int) -> Callable:
        def splitter(sequence: str) -> Tuple[str, str, str]:
            prefix_len = np.random.randint(min_prefix_len, len(sequence) - min_suffix_len)
    ...
    def __call__(self, data: Sequence[str]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # tokenize
        tokenized = [self.tokenizer.tokenize(s) for s in data]

        # flip as needed
        if self.flip_prob > 0:
            maybe_flip = np.random.choice([-1, 1], (len(tokenized),), p=[self.flip_prob, 1 - self.flip_prob])
            tokenized = [s[::f] for s, f in zip(tokenized, maybe_flip)]

        is_flipped = lambda i_: self.flip_prob > 0 and maybe_flip[i_] == -1  # noqa: E731

        # FIM as needed
        maybe_fim = np.random.choice([False, True], len(tokenized), p=[1 - self.fim_prob, self.fim_prob])
            ...
            if fim:
                ...
                if fim_mode[i] == "psm":
                    tokenized[i] = self._wrap(
                        self.fim_pid, prefix, self.fim_sid, suffix, self.fim_mid, middle, flipped=is_flipped(i)
                    )
                else:
                    if self.simple_spm:
                        tokenized[i] = self._wrap(
                            self.fim_sid, suffix, self.fim_pid, prefix, self.fim_mid, middle, flipped=is_flipped(i)
                        )
                    else:
                        tokenized[i] = self._wrap(
                            self.fim_pid, self.fim_sid, suffix, self.fim_mid, prefix, middle, flipped=is_flipped(i)
                        )
            else:
        ...
        if self.pad_to_mult is not None:
            max_len = (self.pad_to_mult * np.ceil(max_len / self.pad_to_mult).astype(int)).item()

        out = torch.full((len(tokenized), max_len), self.tokenizer.pad_id, dtype=torch.long)
...
class MSAOAMaskCollator:
    ...
    def __call__(self, batch_msa: "list") -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        depths = torch.tensor([len(msa) for msa in batch_msa])
        lens = torch.tensor([len(msa[0]) for msa in batch_msa])
        max_len = lens.max()
        max_len = pad_to_mult(max_len, self.pad_to_mult)
        max_depth = depths.max()
        max_depth = max_depth if self.pad_to_mult is None else self.pad_to_mult * np.ceil(max_depth / self.pad_to_mult)

        tokenized = [torch.tensor(np.vstack([self.tokenizer.tokenizeMSA(s) for s in msa])) for msa in batch_msa]
        d = torch.tensor(
            [len(msa[:, 1:-1].flatten()) for msa in tokenized]
        )  # flattened msa shapes, excluding START/STOP tokens

        # allocate the  output to fill
        src = torch.full((len(tokenized), max_depth, max_len), self.tokenizer.pad_id, dtype=torch.long)
        targets = src.clone()
        ...
        for i, (length, ts, msa) in enumerate(zip(d, timesteps, tokenized)):
            ...
            mask_arr = torch.zeros(depths[i] * (lens[i] - 2), dtype=torch.bool)  # ignore START/STOP for masking
            mask_idx = np.random.choice(length.item(), ts.numpy(), replace=False)
...
class MSAARCollator:
    ...
    def __call__(self, batch_msa: "list") -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        refactored = [torch.stack([torch.cat((seq, sep_tensor), 0) for seq in msa]).flatten() for msa in refactored]
        max_len = max([len(msa) for msa in refactored])
        max_len = pad_to_mult(max_len, self.pad_to_mult)

        # pre-pad final array
        src = torch.full((len(tokenized), max_len + 1), self.tokenizer.pad_id, dtype=torch.long)  # include start token
        targets = src.clone()
        src[:, 0] = self.tokenizer.start_id

        for i, msa in enumerate(refactored):
            src[i, 1 : len(msa[:-1]) + 1] = msa[:-1]  # ignore last SEP token, offset inputs by 1
            targets[i, 0 : len(msa)] = msa
            targets[i, len(msa) - 1] = self.tokenizer.stop_id  # replace last SEP token with STOP
