...
from evodiff2.constants import MSA_ALPHABET_PLUS, TaskType
from evodiff2.losses import OAMaskedCrossEntropyLoss
...
class OrderAgnosticDiffusionModel(nn.Module):
    def __init__(self, module: nn.Module, padding_id: int, aux_loss_weight: float = 1.0):
        super().__init__()
        self.module = module
    ...
    def forward(
        ...
        ce_loss, nll_loss = self.loss_func(output["logits"], tgt, mask, timestep, input_mask)
...
class ARDiffusionModel(nn.Module):
    ...
    def forward(self, src: torch.Tensor, tgt: torch.Tensor) -> dict:
        ...
        with torch.no_grad():
            ...
            accu = ((pred_tok == tgt[:, 1:]) * (tgt[:, 1:] >= 0)).float().sum() / n_tokens
...
def _create_bytenet(task: TaskType, model_config: dict, pad_id: int) -> Tuple[ByteNetLM, Set[Type[ByteNetBlock]]]:
...
def _get_hf_model(
    ...
    *,
    ...
    if model_config and pretrained:
        ...
        model_config["vocab_size"] = np.ceil(len(MSA_ALPHABET_PLUS) / 8).astype(int).item() * 8
...
def _create_jamba(task: TaskType, model_config: dict, pad_id: int) -> Tuple[nn.Module, Set[Type[nn.Module]]]:
    pretrained = model_config.pop("pretrained", False)
    model = _get_hf_model("ai21labs/Jamba-v0.1", pad_id, pretrained=pretrained, model_config=model_config)
